...
public class GreetingServiceImpl extends RemoteServiceServlet implements GreetingService {
    ...
    public Srp6InitResponse init(Srp6InitRequest initRequest) {
        ...
        getThreadLocalRequest().getSession(true).setAttribute("CurrentSRP6Server", server);
    ...
    @Override
    public Srp6VerificationResponse verify(Srp6VerificationRequest verificationRequest) {
        HttpSession session = getThreadLocalRequest().getSession(true);
        Srp6Server server = (Srp6Server) session.getAttribute("CurrentSRP6Server");
        Srp6VerificationResponse verifyResponse = server.getVerifyResponse(verificationRequest);

        if (verifyResponse == null) {
            System.out.println("Verification failed for " + server.getIdentity());
        } else {
            System.out.println(server.getIdentity() + " logged in with session key " + server.getSecret());
            System.out.println("Sending back M2 value " + verifyResponse.getM2());
        }
